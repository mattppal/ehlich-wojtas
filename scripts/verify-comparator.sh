...
for required_command in cargo git go lake python3 elan; do
...
if ! elan toolchain list | grep -F "$comparator_toolchain" >/dev/null; then
  echo "warning: Comparator pin asks for $comparator_toolchain, which is not installed"
  echo "building Comparator $comparator_commit with $project_toolchain"
  printf '%s\n' "$project_toolchain" > "$comparator_dir/lean-toolchain"
fi
